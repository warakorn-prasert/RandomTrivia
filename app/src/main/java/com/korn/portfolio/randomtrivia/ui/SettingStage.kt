...
private fun OptionEditor(
    ...
    // get question count
    LaunchedEffect(option.category?.id) {  // TODO : move to onValueChange
    ...
    LaunchedEffect(uiState.value) {
        val defaultDifficulty = when (uiState.value) {
            ...
            NetworkUiState.Success -> difficulties.firstOrNull()
        }
        options[idx] = option.copy(difficulty = defaultDifficulty, amount = 1)
    }
    // set max amount to default
    LaunchedEffect(option.difficulty) {  // TODO : move to onValueChange
